...
//
// Initialise random table with seed <inSeed>
//
void GoodRandom::Init(int inSeed)
{
	// Create random table
	for (int i=0; i<static_cast<int>(256+sizeof(uint)); i++)
		mArray[i] = sChaos[(uint8)(inSeed+i)];									// Create semi-random table

	mIndex = (uint8)inSeed;														// Start index
}
...
//
GoodRandom&	GoodRandom::operator=(const GoodRandom& inRHS)
{
	gMemCopy(mArray, inRHS.mArray, sizeof(mArray));
