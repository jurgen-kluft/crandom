...
namespace xcore
{
	...
	void encrypt_counter(xrndsitmo::state& state)
	{
		...
		for (unsigned short i = 0; i < 4; ++i) b[i] = state._s[i];
		for (unsigned short i = 0; i < 4; ++i) k[i] = state._k[i];
	...
	{
		++state._s[0];
		if (state._s[0] != 0) return;
		++state._s[1];
		if (state._s[1] != 0) return;
		++state._s[2];
		if (state._s[2] != 0) return;
		++state._s[3];
	}
	...
	{
		state._k[0] = k0; state._k[1] = k1; state._k[2] = k2; state._k[3] = k3;
		encrypt_counter(state);
	}
